...
const ALLOWED: Record<string, ReadonlySet<string>> = {
  ...
  media_player: new Set([
    ...
    'toggle',
    // Grouping, for `media_player` entities that are not Sonos. Sonos speakers
    // are grouped through the backend's own Sonos path, which knows about
    // coordinators; these are here for anything else on the dashboard.
    'join',
    'unjoin',
  ]),
 ...
 * legitimately: a play verb targets a `media_player`. Listing
 ...
 *
 * A media player will happily play a local file path or an arbitrary URL if
 * you hand it one, which would turn "play this album" into a way to read the
 * host's disk or make it fetch a URL of the caller's choosing.
...
export class ServiceGuard {
  ...
  async call(call: ServiceCall): Promise<string | null> {
    ...
    // (4) `media_id` names what to play. A media player accepts file paths
